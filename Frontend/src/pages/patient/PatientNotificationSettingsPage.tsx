...
export function PatientNotificationSettingsPage() {
  ...
  return (
    <div>
      <Breadcrumbs trail={[{ label: t('notifications.title') }]} />
      {/* PatientShell renders this same title (hidden lg:block) in its own
          sticky header, keyed off notifications.title (not settings.title) —
          shown only below lg so the two never duplicate. */}
