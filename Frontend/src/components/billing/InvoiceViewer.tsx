  ...
  return (
    <div className="invoice-viewer">
      ...
      <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <MetaField label={t('billing.invoiceDate')} value={formatDate(invoice.invoice_date, language)} />
        <MetaField label={t('billing.dueDate')} value={invoice.due_date ? formatDate(invoice.due_date, language) : '—'} />
      </div>
          ...
          <tbody>
            {invoice.items.map((item) => (
              <tr key={item.id} className="border-b border-slate-100">
                <td className="px-2 py-2.5 text-slate-700" dir="auto">{item.description}</td>
                <td className="whitespace-nowrap px-2 py-2.5 text-end text-slate-600">{item.quantity}</td>
                <td className="whitespace-nowrap px-2 py-2.5 text-end text-slate-600">{money(item.unit_price)}</td>
                <td className="whitespace-nowrap px-2 py-2.5 text-end font-semibold text-slate-800">{money(item.line_total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="ms-auto grid max-w-xs grid-cols-2 gap-y-1.5">
        ...
        <span className="text-end text-sm font-medium text-slate-700">{money(invoice.subtotal)}</span>
        <span className="text-sm text-slate-500">{t('billing.discount')}</span>
        <span className="text-end text-sm font-medium text-slate-700">−{money(invoice.discount)}</span>
        <span className="border-t border-slate-200 pt-1.5 text-sm font-bold text-slate-800">{t('billing.total')}</span>
        <span className="border-t border-slate-200 pt-1.5 text-end text-sm font-bold text-slate-800">{money(invoice.total)}</span>
        <span className="text-sm text-slate-500">{t('billing.paid')}</span>
        <span className="text-end text-sm font-medium text-slate-700">{money(invoice.paid_amount)}</span>
        <span className="border-t border-slate-200 pt-1.5 text-sm font-bold text-[#0D9488]">{t('billing.balance')}</span>
        <span className="border-t border-slate-200 pt-1.5 text-end text-sm font-bold text-[#0D9488]">{money(invoice.balance)}</span>
      </div>
              ...
              <tbody>
                {invoice.payments.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100">
                    <td className="px-2 py-2.5 text-slate-600">{formatDateTime(p.paid_at, language)}</td>
                    ...
                    <td className="whitespace-nowrap px-2 py-2.5 text-end font-semibold text-slate-800">{money(p.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
